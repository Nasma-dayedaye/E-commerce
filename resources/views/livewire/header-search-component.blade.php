<div class="wrap-search center-section">
    <div class="wrap-search-form">
        <form action="{{route('product.search')}}" id="form-search-top" name="form-search-top">
            <input type="text" name="search" value="{{$search}}" placeholder="Cherchez ici...">
            ...
            <div class="wrap-list-cate">
                ...
                <ul class="list-cate">
                    <li class="level-0">Toutes les catégories</li>
                    @foreach ($categories as $category)
                        <li class="level-1" date-id="{{$category->id}}">{{$category->name}}</li>
                    @endforeach
                </ul>
            </div>
        </form>
